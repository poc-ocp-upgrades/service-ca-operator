...
var (
	...
	webconsoleBump	= regexp.MustCompile(regexp.QuoteMeta("bump(github.com/openshift/service-ca-operator-web-console): ") + `([\w]+)`)
...
func main() {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		display := fmt.Sprintf("%s [\\#%s](https://github.com/openshift/service-ca-operator/pull/%s)", message, matches[1], matches[1])
